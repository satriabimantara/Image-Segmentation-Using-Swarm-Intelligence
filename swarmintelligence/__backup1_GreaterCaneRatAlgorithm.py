...
class GreaterCaneRatAlgorithm(SwarmIntelligence):
    ...
    def greater_cane_rat_algorithm(self):
        ...
        """
        Tahap 4: GCRA optimization process
        """
        for iteration in range(n_iterations):
            # update parameters
            params = self.__update_parameters(dominant_male, iteration)

            # update gcr position based on eq 3
            agents = self.__update_gcr_position_eq3(agents, dominant_male)

            # exploration vs exploitation
            agents, fitness_scores, dominant_male = self.__exploration_vsv_exploitation(
                agents, fitness_scores, dominant_male, params)

            # apply boundaries, evaluate fitness, and update dominant male
    ...
    def __exploration_vsv_exploitation(self,
                                       agents, fitness_scores, dominant_male, params):
        rho = self.rho
        xmax = self.UPPER_BOUND
        xmin = self.LOWER_BOUND
        ...
        for idx_agent, agent in enumerate(agents):
            if np.random.rand() < rho:
                ...
                new_fitness_agent = fitness_function(
                    self.__apply_boundaries(agents[idx_agent]))
                if new_fitness_agent > dominant_male['fitness']:
                    dominant_male = {
                        'position': self.__apply_boundaries(agents[idx_agent]).copy(),
                        'fitness': new_fitness_agent
                    }
                else:
                    agents[idx_agent] = agents[idx_agent] + C * \
